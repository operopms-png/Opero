...
export default function Page() {
  ...
  async function saveTemplate() {
    ...
    if (editingTemplateId) {
      ...
      const { error } = await supabase.from('marketing_email_templates').insert([{...templateForm, user_id:user?.id, module:moduleFilter}])
      setSavingTemplate(false)
      if (error) { alert(error.message); return }
    }
  ...
  async function save(table: string, data: any, clearForm: () => void, closeForm: () => void) {
    setSaving(true)
    const {data:{user}} = await supabase.auth.getUser()
    const {error} = await supabase.from(table).insert([{...data,user_id:user?.id,module:moduleFilter}])
    setSaving(false)
    if(error){alert(error.message);return}

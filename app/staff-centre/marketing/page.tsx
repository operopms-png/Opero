...
function moduleBadge(m: string) {
  ...
  if (!found) return <span style={{fontSize:10,padding:'2px 7px',borderRadius:4,background:'#F2F4F7',color:'#667085',fontWeight:600}}>&mdash;</span>
...
export default function Page() {
  ...
  useEffect(()=>{
    supabase.auth.getUser().then(async ({data:{user}})=>{
      ...
      const { data: settings } = await supabase.from('integrations').select('marketing_from_email,marketing_from_name').eq('user_id',accountId).single()
  ...
  async function saveSendSettings() {
    ...
    const accountId = await getAccountId(user!)
    const {error} = await supabase.from('integrations').upsert({user_id:accountId, ...sendSettings}, {onConflict:'user_id'})
  ...
  async function loadAll(userId: string) {
    const [c,e,s,a,tpl] = await Promise.all([
      ...
      supabase.from('marketing_emails').select('*').eq('user_id',userId).order('created_at',{ascending:false}),
      supabase.from('marketing_socials').select('*').eq('user_id',userId).order('created_at',{ascending:false}),
      supabase.from('marketing_ads').select('*').eq('user_id',userId).order('created_at',{ascending:false}),
      supabase.from('marketing_email_templates').select('*').eq('user_id',userId).order('created_at',{ascending:false}),
    ])
  ...
  async function saveTemplate() {
    ...
    if (editingTemplateId) {
      ...
      const { error } = await supabase.from('marketing_email_templates').insert([{...templateForm, user_id:accountId, module:moduleFilter}])
      setSavingTemplate(false)
      if (error) { alert(error.message); return }
    }
  ...
  function exportEmailsCsv() {
    ...
    a.href = url; a.download = `marketing-emails-staff-centre-${new Date().toISOString().slice(0,10)}.csv`
    a.click()
  ...
  async function save(table: string, data: any, clearForm: () => void, closeForm: () => void) {
    ...
    const {data:{user}} = await supabase.auth.getUser()
    const accountId = await getAccountId(user!)
    const {error} = await supabase.from(table).insert([{...data,user_id:accountId,module:moduleFilter}])
    setSaving(false)
    if(error){alert(error.message);return}
  ...
  return (
    <div style={{minHeight:'100vh',background:'#F7F8FA',fontFamily:"'Inter',sans-serif"}}>
      ...
      <div style={{padding:'14px 28px',background:'#fff',borderBottom:'1px solid #E4E7EC',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap' as const}}>
        <span style={{fontSize:12,color:'#98A2B3',fontWeight:600}}>Viewing &amp; creating for:</span>
        {MODULE_OPTIONS.map(m=>(
          <button key={m.key} onClick={()=>setModuleFilter(m.key)} style={{padding:'6px 14px',borderRadius:20,border:moduleFilter===m.key?'1px solid '+m.color:'1px solid #E4E7EC',background:moduleFilter===m.key?m.bg:'#fff',color:moduleFilter===m.key?m.color:'#667085',fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>{m.label}</button>
        ))}
      </div>
      <div style={{padding:24}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
          {[{l:'Campaigns',v:campaigns.filter((c:any)=>c.module===moduleFilter).length},{l:'Emails Sent',v:emails.filter((e:any)=>e.module===moduleFilter&&e.status==='Sent').length},{l:'Social Posts',v:socials.filter((s:any)=>s.module===moduleFilter).length},{l:'Active Ads',v:ads.filter((a:any)=>a.module===moduleFilter&&a.status==='Active').length}].map((s:any)=>(

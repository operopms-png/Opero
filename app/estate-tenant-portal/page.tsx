...
// estate_tenancies (not pm_leases) for the tenancy record, and
// estate_rent_schedules (a recurring schedule row) instead of
// pm_rent_payments (a per-instance charge) for Payments.
...
function EstateTenantPortalInner() {
  ...
  async function payNow(scheduleId: string) {
    setPayingId(scheduleId)
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const res = await fetch('/api/pay-rent-estate', {
        method: 'POST',
  ...
  async function loadAll(t: any) {
    const [{ data: tencs }, { data: scheds }, { data: maint }, { data: msgs }, { data: docs }, { data: ann }] = await Promise.all([
      supabase.from('estate_tenancies').select('*').eq('tenant_id', t.id).order('start_date', { ascending: false }).limit(1),
    ...
    if (t.property_id) {
      const { data: prop } = await supabase.from('estate_properties').select('*').eq('id', t.property_id).single()
      setProperty(prop)
    }
    if (t.unit_id) {
      const { data: u } = await supabase.from('estate_units').select('*').eq('id', t.unit_id).single()
      setUnit(u)
    }
  }
  ...
  async function sendTenantMessage() {
    ...
    const { error } = await supabase.from('estate_tenant_messages').insert({
      tenant_id: tenant.id,
      sender: 'tenant',
  ...
  async function submitRenewalRequest() {
    ...
    const { error } = await supabase.from('estate_tenancies').update({
  ...
  async function saveProfile() {
    ...
    const { error } = await supabase.from('estate_tenants').update(profileForm).eq('id', tenant.id)
  ...
  async function submitTicket() {
    ...
    setSubmitting(true)
    const { error } = await supabase.from('estate_maintenance').insert({
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      let t: any = null
      if (viewingTenantId) {
        const { data } = await supabase.from('estate_tenants').select('*').eq('id', viewingTenantId).single()
        t = data
        setIsStaffView(true)
      } else {
        const { data } = await supabase.from('estate_tenants').select('*').eq('portal_user_id', user.id).single()
        t = data
      }
      if (!t) {
        if (!viewingTenantId) {
          ...
          const { data: rows } = await supabase.from('estate_tenants').select('*').eq('user_id', accountId).order('name', { ascending: true })
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F7F8FA', fontFamily: "'Inter',sans-serif", padding: '48px 28px' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#98A2B3', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Estate Agency — Tenant Portal</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {pickerTenants.map(t => (
              <a key={t.id} href={`/estate-tenant-portal?tenant_id=${t.id}`} style={{ background: '#fff', border: '1px solid #E4E7EC', borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textDecoration: 'none' }}>
                <div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F8FA', fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ display: 'flex', gap: 0, padding: '0 28px', background: '#fff', borderBottom: '1px solid #E4E7EC', flexWrap: 'wrap' as const }}>
        {['My Tenancy', 'Payments', 'Maintenance', 'Documents', 'Messages', 'Announcements', 'Renewal', 'Amenities', 'Profile'].map(s => (
      ...
      <div style={{ padding: 24, maxWidth: 700 }}>
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E4E7EC', padding: 16 }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: '#101828', marginTop: 4 }}>£{(tenancy?.rent ?? 0).toLocaleString()}</div>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E4E7EC', padding: 16 }}>
                <div style={{ fontSize: 11, color: '#667085', textTransform: 'uppercase' }}>Tenancy Start</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#101828', marginTop: 4 }}>{tenancy?.start_date ?? '—'}</div>
              </div>
              <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E4E7EC', padding: 16 }}>
                <div style={{ fontSize: 11, color: '#667085', textTransform: 'uppercase' }}>Tenancy End</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#101828', marginTop: 4 }}>{tenancy?.end_date ?? '—'}</div>
              </div>
            </div>
            {tenancy?.document_url && (
              <a href={tenancy.document_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', padding: '9px 16px', borderRadius: 8, border: '1px solid #D0D5DD', background: '#fff', fontSize: 13, fontWeight: 500, color: '#344054', textDecoration: 'none' }}>View Tenancy Agreement</a>
            )}
            {!tenancy && <div style={{ color: '#98A2B3', fontSize: 13 }}>No tenancy on file yet.</div>}
          </div>
        )}
        ...
        {tab === 'Payments' && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 100px', padding: '10px 20px', background: '#F9FAFB', borderBottom: '1px solid #E4E7EC', fontSize: 11, fontWeight: 600, color: '#667085', textTransform: 'uppercase' }}>
              <span>Frequency</span><span>Amount</span><span>Due Day</span><span>Method</span><span>Status</span><span></span>
            </div>
            {schedules.length === 0 ? <div style={{ textAlign: 'center', padding: 60, color: '#98A2B3', fontSize: 13 }}>No payment history yet</div> :
            schedules.map(s => (
              <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 100px', padding: '14px 20px', borderBottom: '1px solid #F2F4F7', fontSize: 13, color: '#344054', alignItems: 'center' }}>
                <span>{s.frequency ?? 'Monthly'}</span>
                <span>£{(s.amount ?? 0).toLocaleString()}</span>
                <span>{s.due_day ? `Day ${s.due_day}` : '—'}</span>
                <span style={{ textTransform: 'capitalize' }}>{(s.method ?? '').replace('_', ' ')}</span>
                <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: s.status === 'Paid' ? '#D1FAE5' : s.status === 'Overdue' ? '#FEE2E2' : '#FEF3C7', color: s.status === 'Paid' ? '#059669' : s.status === 'Overdue' ? '#DC2626' : '#D97706', width: 'fit-content' }}>{s.status}</span>
                {s.status !== 'Paid' && (
                  <button onClick={() => payNow(s.id)} disabled={payingId === s.id} style={{ fontSize: 12, fontWeight: 600, color: '#fff', background: ACCENT, border: 'none', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', opacity: payingId === s.id ? 0.6 : 1 }}>{payingId === s.id ? 'Redirecting…' : 'Pay Now'}</button>
                )}
              </div>
            ))}
            </div>
                  ...
                  <div>
                    ...
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 12px', border: '1px dashed #D0D5DD', borderRadius: 8, cursor: 'pointer', fontSize: 13, color: '#667085' }}>
                      ...
                      <input type="file" accept="image/*" style={{ display: 'none' }} onChange={async e => {
                        ...
                        const url = await uploadAttachment(file, 'estate-tenant-maintenance')
        ...
        {tab === 'Messages' && (
          <div>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E4E7EC', padding: 16, marginBottom: 12, maxHeight: 400, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {messages.length === 0 ? <div style={{ textAlign: 'center', padding: 30, color: '#98A2B3', fontSize: 13 }}>No messages yet — say hello to your agent</div> :
              messages.map(m => (
                <div key={m.id} style={{ alignSelf: m.sender === 'tenant' ? 'flex-end' : 'flex-start', maxWidth: '75%' }}>
                  ...
                  <div style={{ fontSize: 10, color: '#98A2B3', marginTop: 2, textAlign: m.sender === 'tenant' ? 'right' : 'left' }}>{m.sender === 'tenant' ? 'You' : 'Agent'}</div>
                </div>
              ))}
            </div>
        ...
        {tab === 'Renewal' && (
          <div>
            {tenancy?.renewal_status && tenancy.renewal_status !== 'none' ? (
              <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #E4E7EC', padding: 20, maxWidth: 460 }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: '#101828', textTransform: 'capitalize', marginBottom: 8 }}>{tenancy.renewal_status.replace(/_/g, ' ')}</div>
                {tenancy.renewal_notes && <div style={{ fontSize: 13, color: '#667085' }}>{tenancy.renewal_notes}</div>}
              </div>
            ) : !showRenewalForm ? (
        ...
        {tab === 'Amenities' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E4E7EC', padding: 16, gridColumn: '1 / -1' }}>
              ...
              <div style={{ fontSize: 13, color: '#344054', marginTop: 4 }}>{property?.bin_collection_notes ?? 'Not set yet — check with your agent.'}</div>
            </div>
            <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #E4E7EC', padding: 16, gridColumn: '1 / -1' }}>
              <div style={{ fontSize: 11, color: '#667085', textTransform: 'uppercase' }}>Parking</div>
              <div style={{ fontSize: 13, color: '#344054', marginTop: 4 }}>{property?.parking_notes ?? 'Not set yet — check with your agent.'}</div>
            </div>
...
export default function EstateTenantPortal() {
  return (
